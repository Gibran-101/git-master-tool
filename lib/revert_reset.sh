...
revert_reset_master() {
    ...
    reset_commit() {
        ...
        case "$reset_type" in
            ...
            2)
                ...
                ;;
            3)
                stash_answer=$(prompt_with_validation "Do you want to stash changes before hard reset? (yes/ no) ") || return 1
                if [[ "$stash_answer" == "yes" ]]; then
                    git stash
                    echo " Changes stashed before reset"
                    log_json "INFO" "$SCRIPT_NAME" "Stashed before hard reset"
                fi
                git reset --hard "$commit_hash"
                echo " ⚠️ Hard reset completed. All local changes are now gone."
    ...
    revert_commit() {
        ...
        if git revert "$revert_hash"; then
            ...
            echo " ⚠️ Revert failed — maybe due to conflicts. Fix manually."
            log_json "WARNING" "$SCRIPT_NAME" "Revert failed or conflicted for $revert_hash"
        fi
    }
    ...
    main_revert_menu() {
        ...
        case "$user_choice" in
            ...
            *)
                ...
                log_json "ERROR" "$SCRIPT_NAME" "Invalid menu option: $user_choice"
                ;;
        esac
    }
