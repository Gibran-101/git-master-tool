...
logs_master() {
    ...
    access_last_N_commits() {
        ...
        if ! [[ "$counter" =~ ^[0-9]+$ ]]; then
            ...
            log_json "ERROR" "$SCRIPT_NAME" "Invalid number for commit count: $counter"
            return 1
        fi
    ...
    export_logs() {
        user_response=$(prompt_with_validation "Would you like to export logs? (yes/ no) ") || return 1
        ...
        if [[ "$user_response" == "yes" ]]; then
            ...
            case "$log_type" in
                ...
                *)
                    ...
                    log_json "ERROR" "$SCRIPT_NAME" "Invalid log type: $log_type"
                    return 1
                    ;;
            esac

            if [ $? -eq 0 ]; then
                echo "  Logs exported to $filename.txt"
                log_json "SUCCESS" "$SCRIPT_NAME" "Exported $log_type logs to $filename.txt"
            else
                echo "  Failed to export logs."
                log_json "ERROR" "$SCRIPT_NAME" "Export failed for $filename.txt"
            fi

        elif [[ "$user_response" == "no" ]]; then
            echo " Export cancelled."
            log_json "INFO" "$SCRIPT_NAME" "User cancelled log export"
        else
            echo " Invalid input. Please type yes or no."
            log_json "ERROR" "$SCRIPT_NAME" "Invalid export response: $user_response"
            return 1
        fi
    }
    ...
    main_logs_menu() {
        ...
        log_json "INFO" "$SCRIPT_NAME" "User selected log menu option: $selected_option"
        ...
        case "$selected_option" in
            ...
            *)
                ...
                log_json "ERROR" "$SCRIPT_NAME" "Invalid option: $selected_option"
                ;;
        esac
    }
